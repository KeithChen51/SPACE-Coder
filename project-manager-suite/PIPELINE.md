...
| 产物 | 文件名 | 存放位置 | 说明 |
|------|--------|---------|------|
| C 端页面代码 | Vue 3 工程 | `<host>/可操作页面/<工程名>/` | 可交互，mock 数据（仅 C+B） |
| B 端控制台页面代码 | Vue 3 工程 | `<host>/可操作页面/<工程名>/` | 可交互，mock 数据 |
| 实体中间文件 | `page-spec-entities-<slug>.md` | `<host>/可操作页面/` | C 端实体规格（仅 C+B） |
| 交付清单 | `page-delivery-<slug>.md` | `<host>/可操作页面/` | 页面路由表、文件路径、下游索引 |
...
| 产物 | 文件名 | 存放位置 | 说明 |
|------|--------|---------|------|
| 用户流程图 | `explainer-flow-<slug>.md` | `<host>/可操作页面/` | 按用户任务组织的流程描述（只含流程语义，不含索引） |
| C 端交互描述 | `explainer-c-interaction-<slug>.md` | `<host>/可操作页面/` | 结构化语义条目，含 locked/open 状态（仅 C+B） |
| B 端交互描述 | `explainer-b-interaction-<slug>.md` | `<host>/可操作页面/` | 结构化语义条目，含 locked/open 状态 |
| B 端权限矩阵 | `explainer-b-permission-<slug>.md` | `<host>/可操作页面/` | 角色 × 页面/菜单可见性 |
| C 端差异 | `explainer-c-gap-<slug>.md` | `<host>/可操作页面/` | 分类差异条目（仅 C+B，有差异时） |
| B 端差异 | `explainer-b-gap-<slug>.md` | `<host>/可操作页面/` | 分类差异条目（有差异时） |
| 交付清单 | `explainer-delivery-<slug>.md` | `<host>/可操作页面/` | 产物索引 + 冻结统计 + 差异摘要 + 流程映射 + 一致性自查；本环节收官与下游入口 |
...
- 迁移既有文件时，按 slug 归属关系移动到对应目录即可，内容无需改动。
