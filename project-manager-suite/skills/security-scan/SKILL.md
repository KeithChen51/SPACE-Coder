---
name: security-scan
description: Use when the user is preparing for production release, asking whether a project is safe to go live, requesting a pre-release security gate, or needing a fixed PASS/BLOCK/WAIVER decision before deployment. Use this whenever the request mentions 上线、发版、生产发布、go-live、安全放行、release blocking, or final security checks, even if the user only explicitly mentions network exposure, dependencies, or secrets.
---
...
把“生产上线前安全扫描”变成固定结构的发布闸门，而不是开放式安全顾问。这个 skill 的核心价值是稳定触发、稳定输出、稳定判定，避免在真正发版前出现“没扫就放行”或“每次回答口径都不一样”的情况。

## Gate Rule

命中以下任一条件时，默认进入本 skill：

- 用户要求上线、发版、发布生产、go-live
- 用户要求判断“能不能上线”
- 用户要求做发布前 security gate
...
- 未完成扫描前，不得直接给出“可上线”结论。
- 不得把网络扫描当作全部安全扫描；如果只扫描部分域，必须明确标注为局部扫描。
