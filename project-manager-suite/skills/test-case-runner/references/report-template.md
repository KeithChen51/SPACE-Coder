...
按**测试人员 → 区块**两层组织，参照宿主项目规则文件中的团队/身份约定确定 git 用户名。
...
```
reports/hyhmacstudio/screenshots/测试验收-模块2-状态展示/CTC-STA-11.png
```

区块报告中引用截图的相对路径：
```markdown
[截图](screenshots/测试验收-模块2-状态展示/CTC-STA-11.png)
```

defects.md 中引用截图的相对路径（从 reports/ 根目录出发）：
```markdown
[截图](hyhmacstudio/screenshots/测试验收-模块2-状态展示/CTC-STA-11.png)
```
