...
**触发条件**：当前任务类型为「新建计划」，且宿主 `docs/tech-stack-<slug>.md` 不存在时。

- **不存在时**：进入 [Step 0.25：技术栈确认访谈](#step-025技术栈确认访谈)，访谈完成后生成文件，再继续后续判断。
- **已存在时**：跳过访谈，直接进入 [Step 0.5-trigger：先判断任务类型](#step-05-trigger先判断任务类型)，并在计划头部驾驶舱摘要中补写 `技术栈文件` 字段路径。

---

### Step 0.25：技术栈确认访谈

访谈采用**逐问逐答**方式，一次只抛出一个问题，不给多问题问卷。共 5 个主问题及条件追问：

**1. 前端技术栈（确认而非重选）**
问：page-designer 在页面设计阶段用的是 [Vue 3，或实际读取到的框架]，这个项目继续沿用这个前端框架吗？如果有变化，请告诉我实际用的是什么。（如项目没有前端，回答"无前端"）

**2. 服务端技术栈**
问：后端服务用什么技术栈？（例：Spring Boot 3.x + Java 17、Node.js + Express、Django、无后端等）

**3. 数据库**
问：项目使用什么数据库？（例：MySQL、PostgreSQL、MongoDB、SQLite、无数据库）

**3a. 数据库连接信息（仅当有数据库时进行条件追问）**
问：请提供数据库连接信息（连接串 / host + port + 用户名 + 密码）。
⚠️ **提示用户**：这些信息不会写入计划文件，只会追加写入宿主项目的 `.env.local`，并且需要被忽略。

**4. 部署方式**
问：这个项目是部署到云端服务器，还是本地/私有环境运行？

**4a. 云端部署（仅当选云端时进行条件追问）**
问：请提供服务器连接信息（云厂商、SSH 地址/用户名/密钥路径或密码）。
⚠️ **提示用户**：同上，只追加写入 `.env.local`，不进计划文件。

**4b. 数据库托管方式（云端 + 有数据库时进行条件追问）**
问：数据库是用云服务商托管实例（如 RDS、Cloud SQL），还是自建在服务器上？
