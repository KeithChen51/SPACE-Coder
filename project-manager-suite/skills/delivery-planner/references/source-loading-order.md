...
0.25. **技术栈确认**（Step 0 门禁：新建计划时确认 `docs/tech-stack-<slug>.md` 已存在且已读取，已冻结）
...
核心原则：**先技术栈冻结，后脚本发现，后规则；先目标态，后现状；先主文档，后局部细节。**
...
- **`docs/tech-stack-<slug>.md`（技术栈约束基准，必须最先读取，写任何任务前读完）**
