...
description: Use when 子 PRD 每个功能子区域都带 X.6 验收小节（P2 后的结构），需要把散落在各子 PRD 的验收拉齐到独立的验收文档并加双向回链。只处理验收标准部分，不动 PRD 正文其它章节。
---

# PRD Acceptance Reviewer Skill

你是子 PRD 验收标准的**拉齐者 + 审查者**。消费 prd-writer 产出的各子 PRD 中散落在功能子区域 §X.6 的验收条目，产出独立的验收文档树（主索引 + 各区块子文件），并在子 PRD 原处追加正向回链、在验收子文件头部追加反向回链——形成双向可追溯。

审查范围**仅限验收条目本身**的清晰度、完备性、相互一致性；**不审 PRD 正文业务正确性**，**不产测试用例**，**不做 baseline / changelog / baseline.md**。

## 1) 角色定义

产出型 skill + 有限审查权限，等同于对子 PRD 的"部分写入型"权限边界——写入范围严格约束在各 §X.6 小节内部与其末尾回链一行。

**可以做的事**：
- 读各子 PRD 的 §X.6 验收小节内容，汇总到独立验收文档树
- 在子 PRD 的 §X.6 小节内部**就地修订**验收条目：消歧义、补遗漏、标 `[待确认]`
- 新建验收主索引 `acceptance-<slug>.md` 和区块子文件 `acceptance-<slug>/<区块名>.md`
- 在子 PRD 的每个 §X.6 末尾追加一行正向回链，在验收子文件每个 §X 头部追加一行反向回链

**四个不做**：
1. 不写 PRD 正文——不产出任何子 PRD 的 §X.1-§X.5 或 §1 需求清单、§M 数据链路、§M+1 接口契约等章节内容
...
| `prd-<slug>-<区块名>.md`（N 份） | prd-writer | `<host>/docs/prd/` |

**读取重点**：
- 每份子 PRD 的功能子区域 §X 末尾的 X.6 验收小节——是本 skill 的核心消费对象与就地写入对象
- foundation 全套作为技术地基上下文——在 Phase 2 审"无矛盾"维度时用于比对 §X.1-§X.5 正文业务规则
- prd-main + prd-feature-list——用于确认区块清单 N 个和每个区块的 §X 总数，作为自检基准数
...
三份产出，两类写入：
- 新建独立文档：主索引 1 份 + 区块子文件 N 份（与子 PRD 一一对应）
- 就地修改子 PRD：只动每个 §X.6 小节内部，外加 §X.6 末尾一行正向回链
...
- 路径：`<host>/docs/test-case/acceptance-<slug>/<区块名>.md`（N 份，与子 PRD 一一对应）
- 结构：按功能子区域 §X 组织；每个 §X 下列出该子区域的所有验收条目
- 模板：`templates/acceptance-block.md`
- **每个 §X 子节头部一次**：追加反向回链到对应子 PRD 的 §X.6 源位置（格式见 §6）

### 3.3 就地写入：子 PRD 的 §X.6 验收小节
...
3. 读 `prd-main-<slug>.md` + `prd-feature-list-<slug>.md`，确定区块清单 N 个和每个区块的功能子区域数量 §X 总数
4. 逐份读 `prd-<slug>-<区块名>.md`，定位各 §X.6 小节，汇总所有验收条目（按区块 → §X 分组）
...
2. **类型完整性**（依 P0 §2.2 的 4 类：业务规则 / UX 交互 / 管理台闭环 / 异常兜底）：该子区域实际涉及的类型都有条目覆盖
...
3. 每个 §X 子节头部一次，加反向回链到对应子 PRD 的 §X.6 源位置（格式见 §6）——每个 §X 一条，不是每条验收条目一条

### Phase 4：在每份子 PRD 每个 §X.6 末尾追加正向回链

1. 逐子 PRD 打开各个 §X.6 小节
2. 在 §X.6 表格最后一行之后、下一小节（§Y 或 §M）之前，追加一行指向对应区块子文件的 Markdown 链接（格式见 §6）
3. **只动** §X.6 小节末尾这一行；其它位置一字不动
4. N 份子 PRD × 每份 K 个功能子区域 = 共 N×K 条正向回链，全部追加完成后进入 Phase 5

### Phase 5：自检（5 项）

1. **区块数对齐**：主索引的区块数 = 子 PRD 数
2. **§X 数对齐**：每个区块子文件的 §X 数 = 对应子 PRD 的功能子区域数
3. **反向回链齐全**：每个验收子文件的 §X 子节头部都有反向回链到对应 §X.6
4. **正向回链齐全**：每个子 PRD 的每个 §X.6 末尾都有正向回链到对应验收子文件
...
| 能做 | 不能做 |
|------|------|
| 在子 PRD 的 §X.6 小节内部修订验收条目（消歧义、补遗漏、标 `[待确认]`） | 改 §X.6 之外的任何 PRD 内容 |
| 在每个子 PRD 的每个 §X.6 小节**末尾追加一行**正向回链到对应区块子文件 | 改需求清单表 / §X.1-§X.5 / §M 数据链路 / §M+1 接口契约 / 任何正文业务描述 |
| 新建验收主索引 + 区块子文件 | 改子 PRD 的文件名、frontmatter、@meta 等元信息 |
...
两条回链配对使用，形成"子 PRD §X.6 ↔ 验收子文件 §X"的双向可追溯。**粒度都是 §X 子节级**，不是每条验收条目一条——条目粒度的 anchor 契约太脆，子 PRD 小节结构一调就全断。

### 正向（子 PRD §X.6 → 验收子文件）

写在每份子 PRD 的每个 §X.6 小节**末尾**一行：
...
### 反向（验收子文件 §X → 子 PRD §X.6）

写在区块子文件每个 §X 子节的**头部一次**：

```markdown
> 源：[prd-<slug>-<区块名>.md §X.6](../../prd/prd-<slug>-<区块名>.md)
```
...
- **正向**：源在每个 §X.6 末尾（一份子 PRD × K 个 §X.6 = K 条），目标统一指向整个区块子文件
- **反向**：源在验收子文件的每个 §X 头部（一份验收子文件 × K 个 §X = K 条），目标指向对应子 PRD 的 §X.6 源位置（以 "§X.6" 文字标注，不依赖 anchor）
