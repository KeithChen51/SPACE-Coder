...
| **当前阶段为 S1（业务需求文档）** | 路由到需求文档能力 | **是** | 业务需求文档 |
...
| **当前阶段为 S6（测试执行）** | 路由到测试执行能力 | **是** | 测试结果 / 缺陷清单 |
...
|----------------------------------|----------|--------------|
| `requirements` (业务需求文档) | S1 | `skills/requirements-starter/` |
...
| `test-acceptance` (测试执行、点检准备、验收收口) | S6 / S7 / S8 | `skills/test-and-acceptance/` |
