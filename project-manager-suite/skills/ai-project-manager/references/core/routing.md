...
| **当前轮目标已收敛到页面 / 原型，但“页面任务必补字段包”未补齐** | 停留主入口主动补齐 `项目端别范围`、`当前任务目标端别`、`当前页面主要使用对象` | 否 | 页面任务识别信息补齐结果 |
...
0. 先确认 `项目端别范围`、`当前任务目标端别`、`当前页面主要使用对象` 已补齐
1. 先加载 `ui-ux-pro-max` 生成页面原型或页面代码
2. 将页面原型交给用户确认
3. 用户确认通过后，再加载 `prd-writer` 反推并沉淀完整版 PRD

禁止：
- 不得在页面端别或页面主要使用对象未明确时直接进入 `ui-ux-pro-max`
