...
| S0.5 | `audit_existing_visual_system` / `existing-system-audit` | `project-baseline-auditor` | `dry-run` | 存在既有前端或设计系统证据；只允许 `manage-visual-system extract --dry-run` |
...
后续边界固定：S0/S1/S3 不写文件；S0.5 不执行 adopt/migrate；S4 不写、删除或更新 baseline；S5 的 commitments 不是第二个测试 Oracle；S6 只提供证据，不替代可见浏览器执行、runner 报告、`startCommand` 或 baseline 更新。后端/非 UI 任务和 S7 不加载设计顾问伴随动作。
