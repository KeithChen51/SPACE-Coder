...
const BASELINE_ROOT = SKILL_SOURCE ? resolve(ROOT, "../../evals/design-consultant/visual-baselines/v0.10.0") : join(SCRIPT_DIR, "visual-baselines");
...
async function readBrandAttributionColors(page, accentScope = "focus-and-orbit") {
  return page.evaluate((scope) => {
    ...
    if (!signature || !accentBack || !accent || !focus || !neutral) throw new Error(`Brand attribution ${scope} color layers were not rendered.`);
    ...
    return {
      accentBack: accentBackStyle.backgroundColor,
      accent: accentStyle.backgroundColor,
      expectedAccent: resolveColor(signatureStyle.getPropertyValue("--brand-attribution-accent")),
      focus: focusStyle.backgroundColor,
      neutral: neutralStyle.backgroundColor,
      expectedNeutral: resolveColor(signatureStyle.getPropertyValue("--brand-attribution-neutral")),
...
async function assertInteractions(page) {
  ...
  if (harborLight.accentBack !== harborLight.expectedAccent || harborLight.accent !== harborLight.expectedAccent || harborLight.focus !== harborLight.expectedAccent || harborLight.neutral !== harborLight.expectedNeutral) throw new Error(`Brand attribution did not resolve default semantic colors: ${JSON.stringify(harborLight)}`);
  ...
  if (coralLight.accent !== coralLight.expectedAccent || coralLight.accent !== harborLight.accent || coralLight.neutral === harborLight.neutral) throw new Error(`Brand attribution default accent should stay indigo while neutral follows the host theme: ${JSON.stringify({ harborLight, coralLight })}`);
  await page.getByLabel("自定义署名重点色").fill("#C24135");
  const customLight = await readBrandAttributionColors(page);
  if (customLight.accent !== customLight.expectedAccent || customLight.accent === coralLight.accent) throw new Error(`Brand attribution custom accent did not override the default token: ${JSON.stringify({ coralLight, customLight })}`);
  await page.getByRole("button", { name: "恢复默认靛蓝重点色" }).click();
  const restoredLight = await readBrandAttributionColors(page);
  if (restoredLight.accent !== coralLight.accent) throw new Error(`Brand attribution default indigo was not restored: ${JSON.stringify({ coralLight, restoredLight })}`);
  ...
  if (coralDark.accent !== coralDark.expectedAccent || coralDark.neutral !== coralDark.expectedNeutral || coralDark.neutral === coralLight.neutral || coralDark.accent === coralLight.accent) throw new Error(`Brand attribution did not follow dark semantic colors: ${JSON.stringify({ coralLight, coralDark })}`);
