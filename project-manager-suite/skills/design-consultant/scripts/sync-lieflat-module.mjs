...
function replaceRemoteRuntimes(source) {
  return source
    .replace(/https:\/\/cdn\.jsdelivr\.net\/npm\/chart\.js@[^"'\s<]+/gi, "runtime/chart.umd.min.js")
    .replace(/https:\/\/cdn\.jsdelivr\.net\/npm\/echarts@[^"'\s<]+/gi, "runtime/echarts.min.js");
}
...
for (const file of RUNTIME_FILES) {
  await writeOrCheck(join(OUTPUT_ROOT, "runtime", file), await readFile(join(RUNTIME_ROOT, file), "utf8"), mismatches);
}
