...
async function resolveLayout() {
  ...
  if (await readOptional(sourceManifest)) {
    return {
      ...
      runtimeRoot: resolve(MODULE_ROOT, "vendor/runtime-libs"),
  ...
  return {
    ...
    runtimeRoot: resolve(MODULE_ROOT, "visualizations/lieflat/runtime"),
...
const [manifestText, preview, catalogSource, catalogBundle, tokens, theme, upstreamText, runtimeText] = await Promise.all([
  ...
  readOptional(resolve(layout.runtimeRoot, "RUNTIME.json")),
]);
...
if (!runtimeText) issues.push(`本地运行时清单缺失：${publicPath(resolve(layout.runtimeRoot, "RUNTIME.json"))}`);
...
if (runtime?.dependencies) {
  for (const dependency of runtime.dependencies) {
    const path = resolve(layout.runtimeRoot, dependency.file);
