...
固定依赖的版本、许可证和 SHA256 记录在 `vendor/runtime-libs/RUNTIME.json`，这是 Skill 源码与发布包中的唯一运行时来源。源码 Gallery 直接读取该目录；项目脚手架在生成 `design-system/` 时复制到 `visualizations/lieflat/runtime/`。校验器会拒绝远程 CDN、版本漂移和哈希不一致。
...
| `templates/visualization-lieflat/` | 与通用基线融合的真实 gallery 与主题桥；运行时统一读取 `vendor/runtime-libs/` |
