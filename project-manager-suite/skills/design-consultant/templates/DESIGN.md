...
主品牌 Logo 与 `Powered by` / 技术提供方署名分开记录。没有署名需求时填写 `none`，不要因为模板存在而默认添加。

- **主品牌 Logo：** [身份区位置 + 批准资产路径]
- **署名关系与完整文案：** [none / Powered by SPACE AI Native / 技术提供方 / 合作品牌]
- **署名资产：** [后轨道、S/P/C/E、A、前轨道蒙版、品牌字体及版本化路径]
- **署名重点色：** 默认 light = `#4F46E5`、dark = `#818CF8`；[沿用默认 / 项目通过 `--brand-attribution-accent` 覆写为 … + 选择理由]。该 token 独立于页面 `--primary`
...
- **标准尺寸：** `standard-stacked` = [SPACE 160px / Powered by 12px / AI NATIVE 10px / 纵向间距 7px]；`compact-horizontal` = [SPACE 108px / Powered by 11px / AI NATIVE 10px / 横向间距 12px]
- **字体约束：** `Powered by` 使用项目普通 UI 字体；`SPACE` 图形与 `AI NATIVE` 使用随附品牌字体或批准资产
- **色层约束：** 绘制顺序固定为“后轨道 → S/P/C/E → A → 前轨道”，让 A 遮挡后轨道且前轨道保持跨越；双轨道始终使用 `--brand-attribution-accent`；A 根据统一 `accentScope` 使用重点色或 `--brand-attribution-neutral`；S/P/C/E 与辅助文案使用中性色；inverse 使用 `--brand-attribution-inverse`
- **资产约束：** 使用批准、版本化资产；不重画、不裁切、不拉伸、不用 CSS filter 整体变色；只使用从批准 SVG 确定性生成且几何一致的中性 / 重点色蒙版；`mark-only` 不属于完整署名变体
