...
- 如果存在 `Powered by` / 技术署名，先在 `catalog/component-library.html#brand-attribution` 复核主版 `standard-stacked`、次版 `compact-horizontal`、品牌原生版 `focus-and-orbit`、克制融入版 `orbit-only`、颜色模式与落位示例，再完成桌面、移动、认证 / 授权、已登录首页及明确无 Shell fallback 的 placement map。独立产品、低频品牌表面或需要明确技术归属时优先品牌原生版；成熟主品牌、密集工作台或长期常驻位置优先克制融入版，并在 `DESIGN.md` 记录依据。通过共享 `BrandAttribution` 和批准资产渲染；默认重点色为 light `#4F46E5`、dark `#818CF8`，项目可通过 `--brand-attribution-accent` 独立覆写，但不自动跟随 `--primary`。统一选择一种 `accentScope`，验证同一视口只有一个可见实例，且产品中有一个稳定署名位置；没有需求时明确记录 `none`。
