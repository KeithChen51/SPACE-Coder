...
function inferReadme(files) {
    return files.find((file) => /^readme(\.[a-z]+)?$/i.test(path.basename(file.relativePath))) || null;
}
...
function collectCodeEvidence(files) {
    ...
    const pages = codeFiles.filter((file) =>
        /(^|\/)(pages|views|screens|routes)\//i.test(file.relativePath) ||
        /\.(vue|tsx|jsx)$/i.test(file.relativePath)
    );
    const apis = codeFiles.filter((file) =>
        /(^|\/)(api|apis|routes|controllers|server|backend)\//i.test(file.relativePath) ||
        /controller|route|handler|endpoint/i.test(path.basename(file.relativePath))
    );
    const models = codeFiles.filter((file) =>
        /(^|\/)(models|model|entities|entity|schemas|schema|migrations|database|db)\//i.test(file.relativePath) ||
...
- 其他可用材料：\`【系统推断】 页面 ${evidence.pages.length} 个，接口线索 ${evidence.apis.length} 个，数据模型线索 ${evidence.models.length} 个，配置线索 ${evidence.configs.length} 个\`
...
function buildArtifacts({ profileExistsAfterWrite, docs, evidence }) {
    ...
    const artifacts = [
        ...
        },
        {
            ...
            reason: pagePresent
                ? '已发现页面交互说明文件'
                : `缺少页面流程与交互语义说明；代码中发现页面线索 ${evidence.pages.length} 个`
        },
        {
            type: 'FOUNDATION',
            ...
            reason: foundationPresent
                ? '已发现 foundation 文件'
                : `缺少术语表、Schema、API 与交付清单；代码中发现接口线索 ${evidence.apis.length} 个、数据模型线索 ${evidence.models.length} 个`
        },
        {
            type: 'PRD',
