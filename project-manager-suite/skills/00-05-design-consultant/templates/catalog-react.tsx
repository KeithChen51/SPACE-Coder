...
declare global {
  interface Window {
    __DC_CATALOG__?: {
      ...
      visualizationDocuments?: Readonly<Record<string, string>>;
    };
  }
}
...
function ComponentCatalog({ search, theme }: { search: string; theme: Theme }) {
  ...
  const [signatureAccentInput, setSignatureAccentInput] = useState("");
  const [signatureAi, setSignatureAi] = useState<string | null>(null);
  const [signatureAiInput, setSignatureAiInput] = useState("");
  ...
  useEffect(() => {
    if (signatureAccent) {
      setSignatureAccentInput(signatureAccent);
      return;
    }
    const token = getComputedStyle(document.documentElement).getPropertyValue("--brand-attribution-accent");
    setSignatureAccentInput(normalizeColorInput(token));
  }, [signatureAccent, theme]);

  useEffect(() => {
    if (signatureAi) {
      setSignatureAiInput(signatureAi);
      return;
    }
    const token = getComputedStyle(document.documentElement).getPropertyValue("--brand-attribution-ai");
    setSignatureAiInput(normalizeColorInput(token));
  }, [signatureAi, theme]);

  const signatureStyle = signatureAccent || signatureAi
    ? ({
        ...(signatureAccent ? { "--brand-attribution-accent": signatureAccent } : {}),
        ...(signatureAi ? { "--brand-attribution-ai": signatureAi } : {}),
      } as CSSProperties)
    : undefined;
  ...
  return (
    <>
      ...
      <Section
        ...
        description="保留正式资产的字形与构图，把颜色、结构和落位收敛为可复用的产品组件。"
        count={4}
        countLabel="组规范"
      ...
      >
        <DemoCard
          title="主次结构"
          description="Powered by、SPACE、AI 与 NATIVE 全部使用正式 SVG 字形轮廓；两种结构只调整构图，不重新排字。"
          ...
          <div className="brand-attribution-showcase">
            <div className="brand-attribution-showcase__stage">
              <div className="brand-attribution-showcase__sample" data-size="standard">
                <span className="brand-attribution-spec-label">标准竖向 · 主版本</span>
                <BrandAttribution variant="standard-stacked" />
              </div>
              <div className="brand-attribution-showcase__sample" data-size="compact">
                <span className="brand-attribution-spec-label">紧凑横向 · 次版本</span>
            ...
            <div className="brand-attribution-scope-grid" aria-label="署名重点色范围">
              <div className="brand-attribution-scope-sample">
                <span className="brand-attribution-spec-label">品牌原生版 · 默认</span>
                <BrandAttribution variant="standard-stacked" accentScope="focus-and-orbit" />
                <small>A 与双轨道共同强调。适合独立产品、低频品牌表面和需要明确技术归属的场景。</small>
              </div>
              <div className="brand-attribution-scope-sample">
                <span className="brand-attribution-spec-label">克制融入版 · 低调</span>
                <BrandAttribution variant="standard-stacked" material="flat" accentScope="orbit-only" />
                <small>A 跟随中性字标，仅强调双轨道，并可使用平面材质降低存在感。适合成熟主品牌、密集工作台和长期常驻位置。</small>
              </div>
            </div>
            <div className="brand-attribution-color-controls" aria-label="署名色层预览">
              <div className="brand-attribution-color-controls__intro">
                <strong>受控色层</strong>
                <p>系统点缀色只进入 SPACE 的批准层；AI 使用独立 token，二者互不连带。</p>
              </div>
              <div className="brand-attribution-color-control">
                <div className="brand-attribution-color-control__copy">
                  <strong>系统点缀色</strong>
                  <code>--brand-attribution-accent</code>
                </div>
                <label className="brand-attribution-color-field">
                  <input
                    type="color"
                    aria-label="自定义 SPACE 重点色"
                    value={signatureAccentInput}
                    onChange={(event) => {
                      setSignatureAccentInput(event.target.value);
                      setSignatureAccent(event.target.value);
                    }}
                  />
                  <output>{signatureAccentInput.toUpperCase()}</output>
                </label>
                <button className="brand-attribution-color-reset" type="button" disabled={!signatureAccent} onClick={() => setSignatureAccent(null)}>恢复默认</button>
              </div>
              <div className="brand-attribution-color-control">
                <div className="brand-attribution-color-control__copy">
                  <strong>AI 独立色</strong>
                  <code>--brand-attribution-ai</code>
                </div>
                <label className="brand-attribution-color-field">
                  <input
                    type="color"
                    aria-label="自定义 AI 色"
                    value={signatureAiInput}
                    onChange={(event) => {
                      setSignatureAiInput(event.target.value);
                      setSignatureAi(event.target.value);
                    }}
                  />
                  <output>{signatureAiInput.toUpperCase()}</output>
                </label>
                <button className="brand-attribution-color-reset" type="button" disabled={!signatureAi} onClick={() => setSignatureAi(null)}>恢复默认</button>
              </div>
            </div>
          </div>
        </DemoCard>
        ...
        <DemoCard
          title="颜色模式"
          description="色调、材质和重点色范围分别配置；正式银灰、灰阶与反向灰阶都由同一套蒙版渲染。"
          keywords="brand attribution color tone material metallic flat grayscale reverse monochrome inverse dark light"
          wide
          typeLabel="Tokens"
        >
          <div className="brand-attribution-tone-grid">
            <div className="brand-attribution-tone" data-surface="light">
              <span className="brand-attribution-spec-label">品牌色 · 银灰材质（默认）</span>
              <BrandAttribution variant="standard-stacked" material="metallic" />
            </div>
            <div className="brand-attribution-tone" data-surface="dark">
              <span className="brand-attribution-spec-label">品牌色 · 平面材质</span>
              <BrandAttribution variant="standard-stacked" material="flat" />
            </div>
            <div className="brand-attribution-tone" data-surface="light">
              <span className="brand-attribution-spec-label">正式灰阶 · 浅色背景</span>
              <BrandAttribution variant="compact-horizontal" tone="grayscale" material="metallic" />
            </div>
            <div className="brand-attribution-tone" data-surface="dark">
              <span className="brand-attribution-spec-label">反向灰阶 · 深色背景</span>
              <BrandAttribution variant="compact-horizontal" tone="grayscale-reverse" material="metallic" />
            </div>
            <div className="brand-attribution-tone" data-surface="light">
              <span className="brand-attribution-spec-label">纯黑 · 平面材质</span>
              <BrandAttribution variant="compact-horizontal" tone="monochrome" material="flat" />
            </div>
            <div className="brand-attribution-tone" data-surface="dark">
              <span className="brand-attribution-spec-label">纯白 · 平面材质</span>
              <BrandAttribution variant="compact-horizontal" tone="inverse" material="flat" />
            </div>
          </div>
          <div className="brand-attribution-size-notes">
            <span><strong>标准：</strong>SPACE <code>160px</code>；Powered by <code>80 × 7px</code></span>
            <span><strong>紧凑：</strong>SPACE <code>108px</code>；Powered by <code>97 × 13px</code></span>
...
function App() {
  ...
  const visualizationDocument = window.__DC_CATALOG__?.visualizationDocuments?.[activeVisualization.file];
  ...
  const visualizationSrcDoc = useMemo(() => visualizationDocument
    ?.replaceAll("__DC_CATALOG_PALETTE__", palette)
    .replaceAll("__DC_CATALOG_THEME__", theme), [visualizationDocument, palette, theme]);
  ...
  return (
    <div className="catalog-shell" data-catalog-density={density} data-nav-open={navOpen}>
      ...
      <main className="catalog-main" inert={navOpen || undefined}>
        ...
        {isVisualization ? <><span className="catalog-visualization-status" aria-live="polite">正在显示 {activeVisualization.label}</span><iframe className="catalog-visualization" title={`${activeVisualization.label} 可视化组件预览`} src={visualizationSrcDoc ? undefined : visualizationSrc} srcDoc={visualizationSrcDoc} /></> : <div className="catalog-workspace"><ComponentCatalog search={search} theme={theme} /></div>}
      </main>
    </div>
  );
}
