...
```text
我把这个任务理解为：[产品/页面类型]，面向[用户/角色]，核心目标是[业务目标]，当前适合走[default/customize/design-system/review]主模式，并命中[consumer-product/growth-conversion/host-native/admin-data-workspace/agent-process-ui/data-visualization/preview_decision/enforcement]横向判断。
```
...
12. C 端旅程：是否包含发现、评估、交易、使用、留存或恢复；是否涉及身份、支付、预约、订阅和移动端导航。
...
适合面向普通用户的发现、内容、社交、交易、预约、订阅、账户、娱乐或设备任务。读取 `references/consumer-product-routing.md`、`templates/consumer-product-manifest.json` 与 `references/consumer-ux-contract.md`。

规则：

- 先选一个主产品类型，最多两个次级叠加类型；按核心任务而不是行业名称选择。
- 先判项目状态和客制化深度，再用本模式丰富 Composition Kit；营销信号不能推导出完整品牌设计系统。
- 项目有成体系 token 或共享组件时，仍先执行既有系统事实提取和 `preserve / augment / migrate` 确认。
- 非 C 端后台任务继续走 `admin-data-workspace`，不得因同一项目含消费者页面而改写后台密度和导航模型。

### growth-conversion

适合价值说明、产品演示、社会证明、定价比较、留资、发布下载、目录发现和沉浸叙事。读取 `references/growth-conversion-patterns.md`。

规则：

- 先写目标和可核验证据，再确定主 CTA 与次 CTA。
- 同时写移动退化、可访问性和禁止暗黑模式，不用固定风格词替代产品判断。
- 涉及价格、身份、支付或订阅时，未确认行为保持草案并向用户确认。
